...
router = APIRouter(dependencies=[Depends(get_authorized_user)], tags=["messages"])
...
async def create_user_message(
    ...
    try:
        await messages_service.create_dialog_message(
            sender_id=current_user.id, recipient_id=user_id, message=message
        )
...
async def create_room_message(
    ...
    try:
        await messages_service.create_room_message(
            sender_id=current_user.id, room_id=room_id, message=message
        )
...
async def fetch_room_recent(
    ...
    try:
        return await messages_service.fetch_room_recent(
            room_id=room_id, skip=pagination.skip, limit=pagination.limit
        )
