...
def _prepare_select_query_for_dialog(
    ...
    return (
        ...
        .where(
            and_(
                MessageDb.sender_pk == sender_pk, MessageDb.recipient_pk == recipient_pk
            )
        )
...
def _prepare_select_query_for_room(room_pk: int, skip: int, limit: int) -> Select:
    return (
        select(MessageDb)
...
class AsyncDBMessagesStorage(AsyncMessagesStorage):
    ...
    async def fetch_dialog_recent(
        ...
        return await _select_query_on_messages(
            session=self.__session, select_query=select_query
        )
    ...
    async def fetch_room_recent(
        ...
        select_query = _prepare_select_query_for_room(
            room_pk=try_cast_participant_id(room_id), skip=skip, limit=limit
        )
        return await _select_query_on_messages(
            session=self.__session, select_query=select_query
        )
